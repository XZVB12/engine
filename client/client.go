...
// The Docker client (and by extension maliceio API client) can be made to to send a request
...
// DOCKER_HOST to set the url to the maliceio server.
// DOCKER_API_VERSION to set the version of the API to reach, leave empty for latest.
// DOCKER_CERT_PATH to load the TLS certificates from.
// DOCKER_TLS_VERIFY to enable or disable TLS verification, off by default.
func FromEnv(c *Client) error {
	if maliceioCertPath := os.Getenv("DOCKER_CERT_PATH"); maliceioCertPath != "" {
		options := tlsconfig.Options{
			CAFile:             filepath.Join(maliceioCertPath, "ca.pem"),
			CertFile:           filepath.Join(maliceioCertPath, "cert.pem"),
			KeyFile:            filepath.Join(maliceioCertPath, "key.pem"),
...
func ParseHostURL(host string) (*url.URL, error) {
	...
	if len(protoAddrParts) == 1 {
		return nil, fmt.Errorf("unable to parse maliceio host `%s`", host)
	}
