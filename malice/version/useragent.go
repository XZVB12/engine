...
// MaliceUserAgent is the User-Agent the Malice client uses to identify itself.
// In accordance with RFC 7231 (5.5.3) is of the form:
//    [malice client's UA] UpstreamClient([upstream client's UA])
func MaliceUserAgent(ctx context.Context) string {
	httpVersion := make([]useragent.VersionInfo, 0, 6)
	httpVersion = append(httpVersion, useragent.VersionInfo{Name: "malice", Version: Version})
	...
	maliceUA := useragent.AppendVersions("", httpVersion...)
	upstreamUA := getUserAgentFromContext(ctx)
	if len(upstreamUA) > 0 {
		ret := insertUpstreamUserAgent(upstreamUA, maliceUA)
		return ret
	}
	return maliceUA
}
...
// string of the form:
//   $maliceUA UpstreamClient($upstreamUA)
func insertUpstreamUserAgent(upstreamUA string, maliceUA string) string {
	charsToEscape := `();\`
	upstreamUAEscaped := escapeStr(upstreamUA, charsToEscape)
	return fmt.Sprintf("%s UpstreamClient(%s)", maliceUA, upstreamUAEscaped)
}
