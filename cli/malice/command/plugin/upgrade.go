...
func newUpgradeCommand(maliceCli *command.MaliceCli) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runUpgrade(maliceCli, options)
		},
	}
...
func runUpgrade(maliceCli *command.MaliceCli, opts pluginOptions) error {
	// ctx := context.Background()
	// p, _, err := maliceCli.Client().PluginInspectWithRaw(ctx, opts.localName)
	// if err != nil {
	...
	//
	// fmt.Fprintf(maliceCli.Out(), "Upgrading plugin %s from %s to %s\n", p.Name, reference.FamiliarString(old), reference.FamiliarString(remote))
	// if !opts.skipRemoteCheck && remote.String() != old.String() {
	// 	if !command.PromptForConfirmation(maliceCli.In(), maliceCli.Out(), "Plugin images do not match, are you sure?") {
	// 		return errors.New("canceling upgrade request")
	// 	}
	// }
	//
	// options, err := buildPullConfig(ctx, maliceCli, opts, "plugin upgrade")
	// if err != nil {
	// 	return err
	// }
	//
	// responseBody, err := maliceCli.Client().PluginUpgrade(ctx, opts.localName, options)
	// if err != nil {
